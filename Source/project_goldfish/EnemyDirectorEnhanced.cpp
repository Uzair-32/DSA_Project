...
AEnemyDirectorEnhanced::AEnemyDirectorEnhanced()
{
    ...
    // Initialize HashMap with capacity for expected enemies
...
void AEnemyDirectorEnhanced::BeginPlay()
{
    ...
    // Get all instances of enemy actors
    auto world = GetWorld();
    UGameplayStatics::GetAllActorsOfClass(world, AEnemy::StaticClass(), PEnemies);

    // Build enemy registry with unique IDs - O(n)
...
TArray<FEnemyPriority> AEnemyDirectorEnhanced::GetSortedEnemiesByThreat()
{
    ...
    // Sort using QuickSort - O(n log n)
...
// Original EnemyDirector functionality maintained below
